...
function getfavorite(){
    fetch(`https://se104-project-backend.du.r.appspot.com/movies/632110348`).then((response)=>{
        return response.json()
    }).then( data=>{
        myfav(data)
    })
...
function delid(id){
    fetch(`https://se104-project-backend.du.r.appspot.com/movie?id=632110348&&movieId=${id}`,{
        method: 'DELETE'
    }).then(response =>{
        if(response.status == 200){
            return response.json()
